...
class IPECNet(nn.Module):
    def __init__(self, nc, padding_type, norm_layer, task='identification'):
        ...
        self.head1 = nn.Sequential(*[
            ResnetBlock(nc[i], nc[i + 1], padding_type, norm_layer)
    ...
    def forward(self, X):  #X: Band3 Band4 Band6 Band3-Band4 Band4-Band6
        ...
        feature = torch.cat([feature1, feature2, feature3, feature4, feature5],
                            dim=1)
...
class ResnetBlock(nn.Module):
    def __init__(self, nc_input, nc_output, padding_type, norm_layer):
        ...
        self.conv_block = self.build_conv_block(nc_input, nc_output,
                                                padding_type, norm_layer)
        self.skip_connect = self.build_skip_connect(nc_input, nc_output,
                                                    norm_layer)
    ...
    def build_conv_block(self, nc_input, nc_output, padding_type, norm_layer):
        ...
        block1 = [
            nn.Conv2d(nc_input, nc_output, kernel_size=3, padding=p),
            norm_layer(nc_output),
            nn.ReLU(True)
        ]

        block2 = [
            nn.Conv2d(nc_output, nc_output, kernel_size=3, padding=p),
            norm_layer(nc_output),
            nn.ReLU(True)
        ]
    ...
    def build_skip_connect(self, nc_input, nc_output, norm_layer):
        conv_block = [
            nn.Conv2d(nc_input, nc_output, kernel_size=1),
            norm_layer(nc_output)
        ]
        return nn.Sequential(*conv_block)
