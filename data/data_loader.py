...
class CustomDatasetDataLoader(object):
    def __init__(self, opt, mode='train', suponly=False):
        opt.mode = mode
...
def CreateDataset(opt):
    ...
    elif opt.dataset == 'goes':
        from .goes_dataset import GOES
        dataset = GOES(data_path, opt)
    else:
        raise ValueError("Dataset [%s] not recognized." % opt.dataset)

    print("===> dataset [%s %s] was created" % (dataset.name(), opt.mode))
    return dataset
