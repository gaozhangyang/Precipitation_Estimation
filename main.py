...
import data.goes_dataset as GOES
...
# data_loaders
print('---------------data loading-------------')
train_loader = CustomDatasetDataLoader(opt, mode='train')
val_loader = CustomDatasetDataLoader(opt, mode='test_C_summer')
# test_loader = CustomDatasetDataLoader(opt, mode='test_C_summer')
...
def criterion(logits, target, issup, aux_loss=None):
    if issup.any():
        if opt.weight:
            ...
            ce = F.cross_entropy(logits[issup,...], target[issup], ignore_index=opt.ignore_index, weight=weights)
        else:
...
def evaluate(model, data_loader, writer, device, num_classes, epoch):
    ...
    with torch.no_grad():
        for batch in metric_logger.log_every(data_loader, 100, header):
            image, target = batch['A'].to(device=device), batch['B'].to(device=device)
