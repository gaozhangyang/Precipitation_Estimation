...
class AEResNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, last_layer='softmax',
        ...
        # Encoder
        model = [nn.Conv2d(input_nc, ngf, kernel_size=3, padding=1), # out_size = input_size
        ...
        model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                    kernel_size=3, stride=2,
                                    padding=1), # out_size = input_size * 2
                norm_layer(int(ngf * mult / 2), affine=True),
                nn.ReLU(True)]
        ...
        # Classifier
        model = [nn.Conv2d(ngf, output_nc, kernel_size=3, padding=1)] # out_size = input_size
...
if __name__ == "__main__":
    ...
    X = torch.rand(8, 3, 375, 375).to(device)
    model = AEResNet(3, 10).to(device)
    output = model(X)
    print(model)